...
weChatFilesPath = r"C:\Users\Administrator\Documents\WeChat Files\wxid_9wrukigt5dp322\FileStorage\File\2021-01"
wb = xw.Book(r"C:\Users\Administrator\Desktop\2021年疫情数据填报\2021年1月21日汇总.xlsx")
...
'''
sht1.range('A1:CF44').color = None# 刚开始恢复空白状态
dpa = sht2.range('A2:A42').value
person = sht2.range('B2:B42').value
dpaNames = sht3.range('A2:A42').value
    ...
    for i in c:
        for day in today:
            if day in i:
                ...
                sht = dpaExl.sheets[0]
                data = sht.range('B4:CF4').value
                for Names in dpaNames:
                    for name in Names.split(","):
                        if name in i:
                            print(i)
                            print(Names.split(",")[0])
                            rangeFlag = "B" + str(dpa_table[Names.split(",")[0]][1:]) + ":"+"CF" + str(dpa_table[Names.split(",")[0]][1:])
